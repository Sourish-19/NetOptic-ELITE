...
// Removed next/font/google import, assuming "Anton" is loaded in global CSS or index.html
...
const RevealLoader = ({
    ...
    useGSAP(
        () => {
            ...
            tl.to(".preloader-item", {
                ...
                .call(() => {
                    if (onTextFadeComplete) onTextFadeComplete();
                }, [], "<")
                .to(".name-text span", { autoAlpha: 0, duration: 0.3 }, `<${textFadeDelay}`)
                .to(preloaderRef.current, { autoAlpha: 0, duration: 0.1 }, "+=0.1");

            if (overlayRef && overlayRef.current) {
                // Animate overlay (Navbar) exactly when bars start moving
                tl.fromTo(overlayRef.current,
                    { y: "-150%" },
                    { y: "0%", duration: 0.5, ease: "power2.out" }, // Match bar duration (0.5) roughly
                    "<" // Sync with start of previous (.preloader-item) animation
                );
            }
        },
    ...
    return (
        <div
            className={cn(
                "fixed inset-0 z-[9999] flex overflow-hidden bg-transparent pointer-events-auto", // Fixed position captures viewport
                className,
            )}
            ref={preloaderRef}
        >
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div className="overflow-hidden">
                    <p
                        ...
                        style={{
                            fontFamily: '"Anton", sans-serif', // Use string font stack
                            fontSize: textSize,
                            color: textColor,
                            fontWeight: "400",
