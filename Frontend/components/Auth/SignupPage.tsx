...
const SignupPage: React.FC<SignupPageProps> = ({ onLogin, onSwitchToLogin, API_BASE_URL, onBack }) => {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-royal-950 text-emerald-50 p-4">
            <div className="w-full max-w-md p-8 bg-royal-900/50 backdrop-blur-xl rounded-2xl border border-emerald-500/20 shadow-2xl">
                ...
                <p className="text-center text-emerald-500/60 mb-8">Join the NetOptic Platform</p>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-emerald-500/80 mb-1">Full Name</label>
                        <div className="relative">
                            <User className="absolute left-3 top-3 w-5 h-5 text-emerald-500/40" />
                            <input
                                type="text"
                                required
                                value={name}
                    ...
                    >
                        {loading ? <Loader className="w-5 h-5 animate-spin" /> : <>Get Started <ArrowRight className="w-5 h-5" /></>}
                    </button>
                </form>
